...
static inline uint8_t PCM_ReadROM(gppcm_t& pcm, uint32_t address)
{
    // the real chip picks a wave ROM out of banks here. Furnace hands the
    // whole thing over as one flat space and lets the host sort it out.
    if (pcm.rom_read == NULL)
        return 0;
    return pcm.rom_read(pcm.rom_user, address & 0xffffff);
}
...
void GPPCM_Write(gppcm_t& pcm, uint32_t address, uint8_t data)
{
    ...
    else if (address == 0x3c)
    {
        ...
        GPPCM_GetConfig(pcm.config, data);
    }
...
uint8_t GPPCM_Read(gppcm_t& pcm, uint32_t address)
{
    ...
    else if (address == 0x3c || address == 0x3e) // status
    {
        ...
        if (address == 0x3e && pcm.irq_assert)
        {
            ...
            // the MCU would be told to drop its IRQ line here
        }
...
{
    GPPCM_Reset(pcm);
    pcm.rom_read = romRead;
    pcm.rom_user = romUser;
}

void GPPCM_Reset(gppcm_t& pcm)
{
    GPPCM_ReadROM romRead = pcm.rom_read;
    void* romUser = pcm.rom_user;
    bool isMk1 = pcm.is_mk1;
    bool oversample = pcm.enable_oversampling;
    bool bypass = pcm.filter_bypass;

    memset(&pcm, 0, sizeof(gppcm_t));

    pcm.rom_read = romRead;
    pcm.rom_user = romUser;
    pcm.is_mk1 = isMk1;
    pcm.enable_oversampling = oversample;
    pcm.filter_bypass = bypass;

    // reg_slots has to start at 1, see the derivation in GPPCM_Write
    pcm.config.reg_slots = 1;
}

static inline void PCM_PostSample(gppcm_t& pcm, int32_t left, int32_t right)
{
    if (pcm.out_count >= 2)
        return;
    pcm.out[pcm.out_count][0] = left;
    pcm.out[pcm.out_count][1] = right;
    pcm.out_count++;
}
...
inline void calc_tv(gppcm_t& pcm, int e, int adjust, uint16_t *levelcur, int active, int *volmul)
{
    // int adjust = ram2[3+e];
...
inline int eram_unpack(gppcm_t& pcm, uint32_t addr, int type = 0)
{
    addr &= 0x3fff;
...
inline void eram_pack(gppcm_t& pcm, uint32_t addr, uint32_t val)
{
    addr &= 0x3fff;
    int sh = 0;
    int top = (val >> 13) & 0x7f;
...
{
    {
        ...
        { // final mixing
            ...
            PCM_PostSample(pcm, samp_l, samp_r);
        ...
        for (int slot = 0; slot < pcm.config.reg_slots; slot++)
        {
            ...
            if (active && (ram2[6] & 1) != 0 && (ram2[8] & 0x4000) == 0 && !pcm.irq_assert && irq_flag)
            {
                ...
                // the MCU would be told to raise its IRQ line here
            }
            ...
            int sample;
            if (pcm.filter_bypass)
            {
                // straight off the interpolator, no filter
                sample = sx20(test);
            }
            else
            {
...
{
    uint32_t freq = pcm.is_mk1 ? 64000 : 66207;
