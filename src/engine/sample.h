...
struct DivSample {
  ...
  // 17: Roland FCE-DPCM, as played by the GP and stored in its wave ROMs.
  // one signed delta byte per sample, then the 4-bit shift codes, one per
  // 16-sample block, two to a byte, low nibble first. the codes start right
  // after the deltas, so blocks 0 and 1 share the byte at dataFCE[samples].
  unsigned char* dataFCE;

  unsigned int length8, length16, length1, lengthDPCM, lengthZ, lengthQSoundA, lengthA, lengthB, lengthK, lengthBRR, lengthVOX, lengthMuLaw, lengthC219, lengthIMA, length12, length4, lengthFCE;
